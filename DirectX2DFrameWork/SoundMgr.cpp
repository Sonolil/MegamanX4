...
HRESULT CSoundMgr::Initialize(void)
{
	//사운드 디바이스 생성
...
void CSoundMgr::SoundPlay(int iIndex /*몇번째 파일을 재생할래?*/, DWORD dwFlag/*무한루프, 한번*/)
{
...
void CSoundMgr::SoundAllStop()
{
	for (vector<LPDIRECTSOUNDBUFFER>::iterator iter = m_vecSoundBuff.begin();
		iter != m_vecSoundBuff.end(); ++iter)
	{
		(*iter)->Stop();
	}
}
